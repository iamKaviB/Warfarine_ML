"""
heartrate Patient Label Prediction REST API
...
def validate_heartrate_data(data, required_days=NUM_DAYS):
    """Validate heart rate input data"""
    errors = []
    ...
    # Validate values - heart rate typically ranges 30-220 BPM
    for key in required_keys:
        if key in data:
            value = data[key]
            if not isinstance(value, (int, float)):
                errors.append(f"{key} must be a number")
            elif value < 30 or value > 220:
                errors.append(f"{key} must be between 30 and 220 BPM, got {value}")
...
def create_features_from_input(data):
    ...
    heartrate_cols = [f'val_{i}' for i in range(1, NUM_DAYS + 1)]
    
    features = {}
    
    # Monthly statistics (29 days)
    features['monthly_mean'] = df[heartrate_cols].mean(axis=1).values[0]
    features['monthly_std'] = df[heartrate_cols].std(axis=1).values[0]
    features['monthly_min'] = df[heartrate_cols].min(axis=1).values[0]
    features['monthly_max'] = df[heartrate_cols].max(axis=1).values[0]
    features['monthly_median'] = df[heartrate_cols].median(axis=1).values[0]
    ...
    # Count features - adjusted for heart rate ranges
    features['days_below_90'] = (df[heartrate_cols] < 50).sum(axis=1).values[0]  # Bradycardia
    features['days_below_95'] = (df[heartrate_cols] < 60).sum(axis=1).values[0]  # Low HR
    features['days_above_97'] = (df[heartrate_cols] > 100).sum(axis=1).values[0]  # Tachycardia
    
    # Volatility
    features['volatility'] = df[heartrate_cols].apply(lambda x: x.diff().abs().mean(), axis=1).values[0]
...
def calculate_monthly_stats(data):
    ...
    heartrate_cols = [f'val_{i}' for i in range(1, NUM_DAYS + 1)]
    values = [data[col] for col in heartrate_cols]
    ...
    stats = {
        ...
        'days_below_50': int(sum(1 for v in values if v < 50)),  # Severe bradycardia
        'days_below_60': int(sum(1 for v in values if v < 60)),  # Bradycardia
        'days_above_100': int(sum(1 for v in values if v > 100)),  # Tachycardia
        'total_days': NUM_DAYS
    }
    
    # Add percentage metrics
    stats['percent_below_50'] = (stats['days_below_50'] / NUM_DAYS) * 100
    stats['percent_below_60'] = (stats['days_below_60'] / NUM_DAYS) * 100
    stats['percent_above_100'] = (stats['days_above_100'] / NUM_DAYS) * 100
    
    # Risk assessment based on monthly stats
    if stats['mean'] < 50 or stats['days_below_50'] > NUM_DAYS * 0.3:
        stats['risk_level'] = 'high'
    elif stats['mean'] < 60 or stats['days_below_60'] > NUM_DAYS * 0.5 or stats['mean'] > 120:
...
def index():
    """API root endpoint"""
    return jsonify({
        'name': 'Heart Rate Patient Label Prediction API',
        'version': API_VERSION,
        'status': 'running',
        ...
        'endpoints': {
            ...
            'POST /api/validate': 'Validate heart rate data'
        },
...
def validate_endpoint():
    """Validate heart rate data without making prediction"""
    try:
        data = request.json
        ...
        # Validate
        errors = validate_heartrate_data(data)
...
def predict_batch():
    ...
    try:
        ...
        for idx, patient_data in enumerate(patients):
            try:
                # Validate
                validation_errors = validate_heartrate_data(patient_data)
    ...
    try:
        ...
        for week in range(4):
            ...
            weekly_analysis.append({
                ...
                'days_below_50': int(sum(1 for v in week_values if v < 50))
            })
...
if __name__ == '__main__':
    print("=" * 80)
    print("Heart Rate Patient Label Prediction REST API")
    print("=" * 80)
